...
class stock_web_module_data(metaclass=singleton_type):
    def __init__(self):
        ...
        self.data_list = [wmd.web_module_data(
            ...
            type="股票形态相似",
            ico="fa fa-tag",
            name=tbs.TABLE_CN_STOCK_KLINE_SIMILAR['cn'],
            table_name=tbs.TABLE_CN_STOCK_KLINE_SIMILAR['name'],
            columns=tuple(tbs.TABLE_CN_STOCK_KLINE_SIMILAR['columns']),
            column_names=tbs.get_field_cns(tbs.TABLE_CN_STOCK_KLINE_SIMILAR['columns']),
            primary_key=[],
            is_realtime=False,
            order_columns=f"(SELECT `datetime` FROM `{tbs.TABLE_CN_STOCK_ATTENTION['name']}` WHERE `code`=`{tbs.TABLE_CN_STOCK_KLINE_SIMILAR['name']}`.`code`) AS `cdatetime`",
            order_by=" `cdatetime` DESC"
        ), wmd.web_module_data(
            mode="query",
            type="股票涨停专题",
            ico="fa fa-tag",
            name=tbs.TABLE_CN_STOCK_LIMITUP['cn'],
            table_name=tbs.TABLE_CN_STOCK_LIMITUP['name'],
            columns=tuple(tbs.TABLE_CN_STOCK_LIMITUP['columns']),
            column_names=tbs.get_field_cns(tbs.TABLE_CN_STOCK_LIMITUP['columns']),
            primary_key=[],
            is_realtime=False
        ),wmd.web_module_data(
            mode="query",
