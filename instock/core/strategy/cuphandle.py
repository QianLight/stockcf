...
while working == True:
    ...
    try:
        ...
        os.chdir(outputPath)
        if os.path.isfile(
                f'{daily_file_name}.pdf'):  # 萬一出現重複值, 自動continue 跳出迥圈, 回到上一個try,向前找出未下載資料既weekday
            continue
        else:
            pass  # 如果沒有, 就繼續
        os.chdir(pythonPath)
        ...
        for stock in stocklist:
            try:
                ...
                try:
                    ...
                    continue
                try:
                    RS_Rating = (((currentClose - close_3m) / close_3m) * 40 + (
                ...
                try:
                    ...
                    if (
                            condition_1 and condition_2 and condition_3 and condition_4 and condition_5 and condition_6 and condition_7 and condition_8 and condition_9 and condition_11):
                        final.append(stock)
                        index.append(n)
                        rs.append(RS_Rating)
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                if (name == 'VOO'):
                    mpf.plot(hist, **kwargs, style='brasil', title=titlename,
                             savefig=dict(fname=outputPath_name + "{}.png".format(filename), dpi=125, pad_inches=0.15))
                    ...
                    mpf.plot(hist, **kwargs, style='blueskies', title=titlename,
                             savefig=dict(fname=outputPath_name + "{}.png".format(filename), dpi=125, pad_inches=0.15))
                    index_list.append(name)
                    stock_name.append(name)
                    stock_ind.append(name)
                    ...
                    mpf.plot(hist, **kwargs, style='mike', title=titlename,
                             savefig=dict(fname=outputPath_name + "{}.png".format(filename), dpi=125, pad_inches=0.15))
                    index_list.append(name)
                    stock_name.append(name)
                    stock_ind.append(name)
                    ...
                    mpf.plot(hist, **kwargs, style='classic', title=titlename,
                             savefig=dict(fname=outputPath_name + "{}.png".format(filename), dpi=125, pad_inches=0.15))
                    index_list.append(name)
                    stock_name.append(name)
                    stock_ind.append(name)
                    ...
                    mpf.plot(hist, **kwargs, style='starsandstripes', title=titlename,
                             savefig=dict(fname=outputPath_name + "{}.png".format(filename), dpi=125, pad_inches=0.15))
                    stock_name.append(name)
                    stock_ind.append(name)
                    stock_ind.append(RS_rank_title)
